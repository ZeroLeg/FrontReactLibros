...
export default class LibroInfoComponent extends Component {
    ...
    componentDidMount(){
        const api_img = `/api/libros/get_imgs/`;
        const api_src = "/api/imagenes/";
        const api_delete = "/api/libros/";

        axios.get("/api/libros/"+this.state.isbn)
        .then( res=>{
                ...
                <Container>
                    <Row>
                        <Col xs={6} className="text-left mt-5">
                            ...
                            <span className="d-block">Website: {this.state.website}</span>
